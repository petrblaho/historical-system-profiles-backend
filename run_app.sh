...
then
  ...
  then
    ...
    if [[ "$?" != "0" ]]; then exit 1; fi
    exec gunicorn wsgi --worker-class uvicorn.workers.UvicornWorker --bind=0.0.0.0:$PORT --bind=0.0.0.0:$METRICS_PORT --log-level=$LOG_LEVEL --limit-request-field_size=$GUNICORN_REQUEST_FIELD_LIMIT --access-logfile=- --config "$APP_CONFIG"
